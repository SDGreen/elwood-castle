...
feature users need to view in a rush and users will also have their order emails.
...
> As a User, I would like consistent styling across the site, so I can navigate across the site without having to think too hard about what elements do.

* All buttons and text-links have consistent styling. Disabled buttons (such as *checkout* and *book* under certain conditions) 
change colour & no longer react to being hovered over so users can tell something has changed
...
and their orders. Order numbers can be clicked to get a full summary of that particular order.
...
#### Adding tickets to a booking in the basket would create new bookings rather than update the current values:
This occured because the basket was storing the event_ids as strings and the event_ids 
coming from the database were integers. For the ticket quantity to be updated,
the event_id from the database had to be convereted to a string before being used to find 
a match in the basket.

#### Basket data could not be stored in the payment intent metadata, this kept failing.
This issue kept occuring and I was convinced it was to do with trying to store the event (as an object not id)
in the payment intent after it had been JSON serialised. After numerous attempts it was discovered the actual 
variable causing the issue was Total. Total was a Decimal and json.dumps() couldn't handle that type of input.
Instead Total was changed to a float and the basket and user data could be stored in the payment intent.
...
#### Contact messages sent via Gmail lose the user's inputted email.
Although the message functionality worked fine in the console, when sending messages via Gmail, the user's 
email is removed from the "from" section and replaced by Elwood's Gmail account.
